...
object ALSEvaluation {
  ...
  def main(args: Array[String]) {
    ...
    sql("use arrival")
    val rawUserAppData = sql("select imei as user,pkg as app,rating,lasttimestamp" +
      " from mid_recommend_rating where year='2017' and month='05' and day='10'")
  ...
  def getBestModel(training: DataFrame, validation: DataFrame): Option[ALSModel] = {
    ...
    val ranks = List(8, 12)
    val lambdas = List(0.01, 0.02, 0.1, 0.2, 10.0)
