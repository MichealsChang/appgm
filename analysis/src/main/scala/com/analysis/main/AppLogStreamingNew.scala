...
import com.analysis.util.{DataUtil, MyMultipleTextOutputFormatNew}
...
/**
  ...
  * @author 白鑫
  */
object AppLogStreamingNew {

  val ARRIVAL_DIR = "/test/new/arrival_data"
  ...
  def main(args: Array[String]) {
    ...
    val sparkConf = new SparkConf().setAppName("AppLogStreamingNew")
    ...
    val kafkaStream = KafkaUtils.createStream(
      ...
      Map[String, Int]("arrivallog_new" -> 0,"arrivallog_new" -> 1), //日志在Kafka中的topic及其分区
    ...
    kafkaStream.foreachRDD((rdd: RDD[String], time: Time) => {
      ...
      result.saveAsHadoopFile(ARRIVAL_DIR, classOf[String], classOf[String], classOf[MyMultipleTextOutputFormatNew[String, String]])
    })
