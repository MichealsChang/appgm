...
/**
  * 从Hive表base_arrival_new中提取信息到新表dim_app_new
  *
  * @author 白鑫
  */
object ExtractAppNew {
...
//    val day = args(0).substring(6, 8)
    ...
    // 设置应用名称，新建Spark环境
    val sparkConf = new SparkConf().setAppName("ExtractAppNew_" + args(0))
    val spark = SparkSession
      .builder()
      .config(sparkConf)
      .enableHiveSupport()
      .getOrCreate()
    println("Start " + "ExtractAppNew_" + args(0))
    ...
    val sqlStr = "select (case when (firstimei != ' ') then firstimei when (secondimei != ' ') then secondimei when (thirdimei != ' ') then thirdimei else null end) as imei, " +
      "applist, arrivetimes, year, month, day from base_arrival_new where year=" + year + " and month=" + month
...
//    val insertSql = "FROM app a\n" + generateInsertSql(year,month)
  ...
  def generateInsertSql(year: String, month: String, day: String): String = {
    ...
    sb.append("INSERT OVERWRITE TABLE dim_app_new\n")
