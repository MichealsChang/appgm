...
import com.analysis.util.RandomUtil
...
object ParseBrush {
  ...
  def main(args: Array[String]) {
    ...
    val fileAndLine = hadoopRdd.mapPartitionsWithInputSplit((inputSplit: InputSplit, iterator: Iterator[(LongWritable, Text)]) => {
      ...
      iterator.map(x => { (file.getPath.toString(), x._2.toString) })
    })
  ...
  def toCpzSource(imeis: (String, String), result: Map[String, String]): CpzSource = {
    ...
    //    param = result.get("imid")//
    //    if (!param.isEmpty)
    imid = null
    //    param = result.get("channelid")//
    //    if (!param.isEmpty)
    channelid = "0"
    param = result.get("phoneid")
    if (!param.isEmpty)
      phonemodel = param.get
    //    param = result.get("cooperationtype")//
    //    if (!param.isEmpty)
    cooperationtype = null
    //    param = result.get("cost")//
    //    if (!param.isEmpty)
    cost = null
    ...
    //    param = result.get("currencytype")//
    //    if (!param.isEmpty)
    currencytype = null
    //    param = result.get("state")
    //    if (!param.isEmpty)
    state = "1"
    ...
    //    param = result.get("ccount")//
    //    if (!param.isEmpty)
    ccount = "0"
    //    param = result.get("chcount")//
    //    if (!param.isEmpty)
    chcount = "0"
    //    param = result.get("simstate")//
    //    if (!param.isEmpty)
    simstate = null
    //    param = result.get("linktype")//
    //    if (!param.isEmpty)
    linktype = null
    //    param = result.get("manufacturid")//
    //    if (!param.isEmpty)
    manufacturid = null
    //    param = result.get("manufacturname")//
    //    if (!param.isEmpty)
    manufacturname = null
    param = result.get("time")
    if (!param.isEmpty)
    ...
    if (!param.isEmpty)
      romtype = param.get
    //    param = result.get("packagelist_copy")//
    //    if (!param.isEmpty)
    packagelist_copy = null
    //    param = result.get("companyid")//
    //    if (!param.isEmpty)
    companyid = null
    //    param = result.get("systemappcount")//
    //    if (!param.isEmpty)
    systemappcount = null
    //    param = result.get("userappcount")//
    //    if (!param.isEmpty)
    userappcount = null
...
//    if (StringUtils.isNotBlank(result.appid))
//      p.addColumn(databytes, Bytes.toBytes("appid"), Bytes.toBytes(result.appid))
